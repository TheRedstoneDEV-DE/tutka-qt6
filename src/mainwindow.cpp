...
bool MainWindow::eventFilter(QObject *watched, QEvent *event)
{
    ...
    bool isWindow = watched->isWindowType();
#else
    bool isWindow = watched == this || watched->inherits("QDialog");
#endif
    if ((event->type() == QEvent::KeyPress || event->type() == QEvent::KeyRelease) && isWindow && dynamic_cast<QLineEdit *>(QApplication::focusWidget()) == NULL && dynamic_cast<QSpinBox *>(QApplication::focusWidget()) == NULL && dynamic_cast<Tracker *>(QApplication::focusWidget()) == NULL && dynamic_cast<QComboBox *>(QApplication::focusWidget()) == NULL) {
    ...
    if (ctrl) {
        ...
        case Qt::Key_Left:
            ...
            if (ui->spinBoxInstrument->value() > 1) {
                ...
                song->checkInstrument(ui->spinBoxInstrument->value() - 1);
            }
            handled = true;
            break;
        case Qt::Key_Right:
        ...
        if (instrument >= 0) {
            ui->spinBoxInstrument->setValue(instrument + 1);

            // Make sure the instrument exists
            song->checkInstrument(instrument);
            handled = true;
        }
    }
...
void MainWindow::setInstrument(int instrument)
{
    if (instrument > 0) {
        ...
        ui->tracker->setInstrument(instrument);
    }
}
...
{
    QMessageBox::about(this, tr("About Tutka"), tr("Tutka 1.1.3\n\n© 2002-2019 Vesa Halttunen <vesuri@jormas.com>"));
}
...
void MainWindow::handleMidiInput(const QByteArray &data)
{
    ...
    case 0x90:
        ...
        if (ui->checkBoxEdit->isChecked()) {
            block->setNote(ui->tracker->line(), ui->tracker->cursorTrack(), data[1] / 12, data[1] % 12 + 1, ui->spinBoxInstrument->value());
...
{
     QRegularExpression regExp("=?([\\d]+)[xX]([\\d]+)([+-][\\d]+)([+-][\\d]+)");
...
{
    player->playNote(ui->spinBoxInstrument->value() - 1, ui->comboBoxKeyboardOctaves->currentIndex() * 12 + note, 127, ui->tracker->cursorTrack());
}
